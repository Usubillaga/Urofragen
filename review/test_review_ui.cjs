...
// Isolate interactive tests from the imported clinical decisions.
for(const q of bank.questions)q.review.imported_decisions=[];
const source=html.match(/<script>([\s\S]*?)<\/script>/)[1];
const elements={};let saved=null,failStorage=false,downloads=[];
class El{
 constructor(tag='div',text='',value=''){this.tag=tag;this.textContent=text;this.value=value;this.children=[];this.events={};this.files=[];this.style={};}
